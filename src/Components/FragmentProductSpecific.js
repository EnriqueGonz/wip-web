  ...
  useEffect(() => {
    axios.get(baseUrl + '/products/api/specific_product/' + id_product + '/', { headers })
      .then((response) => {
        ...
        setDisponibles(response.data[0][["status"]]);
 ...
 /*  const handleSubmit = (event) => {
  ...
  const postPedido = () => {
    ...
    if(user_points >= costo){
      ...
      user: user_id,
      useraddresses: "1",  /* Se dejo estatico para td synnex */
      campaigns: campana_id,
      order_date: date.getFullYear().toString()+"-"+date.getMonth().toString()+"-"+date.getDate().toString(),
  ...
  return (
    ...
    <div>
            ...
            {list.map((item) => (
              <div key={item.id}>
                <div className="form-check">
                  ...
                  <div style={{ display: "inline-flex", alignItems: "center" }}>
                    ...
                    <div style={{display:"flex", alignItems:"center"}}>
                      <button style={{ paddingTop:"0px", fontSize: 25 }} onClick={handleOnRemove} className="btn"><MdRemove /></button>
                      <h4>{amount}</h4>
                      <button style={{ paddingTop:"0px", fontSize: 25 }} onClick={handleOnAdd} className="btn"><MdAdd /></button>
                    </div>
                  </div>
                  ...
                  {/* <Row>
                    ...
                    <Col style={{ textAlign: "right" }}>
                      <Button variant="secondary" onClick={showAddDireccion}>Agregar direccion</Button>
                    </Col>
                  </Row>
                ...
                <div className="container" style={{ paddingRight: 0, textAlign: "right" }}>
                  ...
                  <Button variant="secondary" onClick={postPedido}>Escoge este regalo</Button>
      ...
      <Modal show={show} size="md" >
        <Modal.Body style={{ backgroundColor:  colorheader}}>
          <div>
            <div>
              <h4 style={{ color:colorletra, fontWeight: 600, paddingTop: 15 }}>Pedido realizado con exito</h4>
              <hr style={{padding:" 0px", margin: "0px", height: "3px", color: "white", opacity:".9"}}></hr>
              <p style={{ color:colorletra, fontSize: 24, fontWeight: 300 }}>Inicia sesion para poder ver tus pedidos</p>
            </div>
            <div className="container" style={{ textAlign: "end" }}>
              <Button variant="secondary" style={{ borderRadius: 15, fontWeight:600 }} className="btn" onClick={event => window.location.href = '/login'} >Volver</Button>
            </div>
